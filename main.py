...
class Pokemon:
    ...
    def calculate_stats(self) -> None:
        ...
        hp_stat: int = mt.floor(((2 * temp_base[0] + self.ivs[0] + mt.floor(self.evs[0] / 4)) * self.level / 100) + self.level + 10)
        self.stats.append(hp_stat)
        self.hp = hp_stat
        temp_base.pop(0)
        i: int = 0
        for stat in temp_base:
            other_stat: int = mt.floor((mt.floor((2 * stat + self.ivs[i+1] + mt.floor(self.evs[i+1] / 4)) * self.level) / 100) + 5)
    ...
    def attack(self, move: Move, target) -> None:
        ...
        match move.category:
            case 'physical': rel_atk, rel_def = self.stats[1], target.stats[2]
            case 'special': rel_atk, rel_def = self.stats[3], target.stats[4]
...
def battle(player: Pokemon, opponent: Pokemon) -> None:
    ...
    while player.is_alive() and opponent.is_alive():
        ...
        while not valid_input:
            ...
            print(move_list_str)
            user_input = input()
            if user_input == '1': move, valid_input = player.move_list[0], True
            elif user_input == '2': move, valid_input = player.move_list[1], True
            elif user_input == '3': move, valid_input = player.move_list[2], True
            elif user_input == '4': move, valid_input = player.move_list[3], True
            else: print("Invalid choice.")
        player.attack(move, opponent)

def damage_calc(rel_atk: int, rel_def: int, level: int, power: int | float) -> int:
    damage: int = round(((((((2 * level) / 5) + 2) * power * (rel_atk/rel_def))/50) + 2) * (random.randint(85,100) / 100))
    return damage
...
def move_select(player: Pokemon) -> Move:
    ...
    while not valid_input:
        print("Please select a move:")
        move_list = [player.move_list[0].name, player.move_list[1].name, player.move_list[2].name, player.move_list[3].name]
        delimiter = ', '
        move_list_str = delimiter.join(move_list)
        print(move_list_str)
        user_input = input()
        if user_input == '1': move, valid_input = move_list[0], True
        elif user_input == '2': move, valid_input = move_list[1], True
        elif user_input == '3': move, valid_input = move_list[2], True
        elif user_input == '4': move, valid_input = move_list[3], True
        else: print("Invalid choice.")
        return move
